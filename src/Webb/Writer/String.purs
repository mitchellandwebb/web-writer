module Webb.Writer.String where
...
import Webb.Writer.Array (dropWhileIncl, takeWhileExcl, dropWhileExcl)
...
trimMargin str = flip evalState init do
  ...
  dropMargin = do
    ...
    when hasMargin do
      let withoutMargin = dropWhileIncl notMargin rem
  ...
  takeLine = do
    rem <- mreads _.remaining
    let line = takeWhileExcl notNewline rem
        withoutLine = dropWhileExcl notNewline rem
    mmodify_ $ \s -> s
